...
function addProperties (root, hints, section, page, pageFx) {
  if (!hints) {
    return
  }
  if (!root.properties) {
    ...
    for (const hint of hints) {
      if (pageFx.showWhens) {
        pageFx.showWhens.forEach(showWhen => {
          if (hint.key === showWhen.target) {
            addProperty(root, hint, section, page, showWhen)
          }
        })
        if (!root.properties[section].properties[page].properties[hint.key]) addProperty(root, hint, section, page)
...
function addProperty (root, hint, section, page, showWhen) {
